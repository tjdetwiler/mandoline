use {cgmath::Vector3, stl_loader::StlFile};
...
impl TriangleMesh {
    pub fn from_stl(stl: StlFile) -> Self {
        ...
        static_assertions::assert_eq_size!(Vector3<f32>, [f32; 3]);
        static_assertions::assert_eq_align!(Vector3<f32>, f32);
        let points: Vec<Vector3<f32>> = unsafe {
...
impl<'a> Iterator for Triangles<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.chunks.next().map(|a| Triangle {
            p0: Vector3 { x: a[0], y: a[1], z: a[2] },
            p1: Vector3 { x: a[3], y: a[4], z: a[5] },
            p2: Vector3 { x: a[6], y: a[7], z: a[8] },
        })
    }
}
*/
...
mod tests {
    ...
    fn create_stl_mesh() {
        ...
        let _mesh = TriangleMesh::from_stl(stl);
    }
}
